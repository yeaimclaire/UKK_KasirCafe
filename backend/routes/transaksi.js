...
app.get("/getAllData", auth, async (req, res) => {
  ...
  await transaksi
    .findAll() // mengambil semua data transaksi
    .then((result) => {
      // jika berhasil
      res.status(200).json({
...
app.get("/getById/:id", auth, async (req, res) => {
  // endpoint untuk mengambil data transaksi berdasarkan id transaksi
  await transaksi
    .findByPk(req.params.id) // mengambil data transaksi berdasarkan id transaksi yang dikirimkan melalui parameter
    .then((result) => {
      // jika berhasil
      if (result) {
        res.status(200).json({
...
app.post("/create", async (req, res) => {
  ...
  const data = {
    // inisialisasi data yang akan dimasukkan
  ...
  await transaksi
    ...
    .then((result) => {
      // jika berhasil
      res.status(200).json({
        // mengembalikan response dengan status code 200 dan pesan transaksi berhasil ditambahkan
        status: "success",
  ...
  transaksi
    ...
    .then((result) => {
      ...
      if (result) {
        // jika data ditemukan
        res.status(200).json({
          // mengembalikan response dengan status code 200 dan pesan transaksi berhasil dihapus
          status: "success",
...
app.get("/filter/tgl_transaksi/:tgl_transaksi", auth, async (req, res) => {
  // endpoint untuk mencari data transaksi berdasarkan tanggal transaksi
  ...
  transaksi
    .findAll({
      ...
      where: {
        tgl_transaksi: {
          [Op.between]: [param.tgl_transaksi + " 00:00:00", param.tgl_transaksi + " 23:59:59"], // mencari data transaksi berdasarkan tanggal transaksi yang dikirimkan melalui parameter
        },
      },
      include: [
